#!/usr/bin/env tsx
/**
 * One-shot test bench for the tailored-resume pipeline.
 *
 * Reads a real JD from /tmp/lifemd-jd-clean.txt, runs:
 ...
 *
 * Usage:
 *   ANTHROPIC_API_KEY=... npx tsx scripts/test-lifemd-resume.ts
 *
 * Output:
 *   data/resumes/test-lifemd-<ts>.pdf
 *   /tmp/test-lifemd-<ts>-analysis.json
 *   /tmp/test-lifemd-<ts>-resume.json
 *
 * Once we're happy with the prompt's behaviour this script can be
 * deleted — it's not wired into anything else.
 */
...
// Next.js auto-loads .env.local for API routes; standalone tsx scripts
// don't. We can't use process.loadEnvFile because it refuses to overwrite
// existing env vars — and Claude Code may pre-export an empty
// ANTHROPIC_API_KEY plus an ANTHROPIC_BASE_URL pointing at its local
// relay, both of which we need to clobber for direct API access.
try {
  ...
  // If .env.local doesn't define a base URL, drop any inherited one so
  // the SDK hits api.anthropic.com directly.
...
// --- Inputs (pinned for this test) -----------------------------------------

const JD_PATH = "/tmp/lifemd-jd-clean.txt";
const JOB_TITLE = "Flutter Application Developer";
const COMPANY = "LifeMD";
const JOB_URL = "https://www.linkedin.com/jobs/view/4401340089/";
...
// Project root — where data/resumes/ and scripts/generate-pdf.js live.
const PROJECT_ROOT = path.resolve(
  path.dirname(fileURLToPath(import.meta.url)),
  ".."
);

// --- Profile context (mirrors AIAgent.getProfileContext) -------------------
...
// --- Step 1: analyzer (Saqr-equivalent) ------------------------------------
...
Schema:
{
  ...
  "matchedSkills": [{"skill": "Flutter", "evidence": "5+ years across Trivia, Nologystore, Famcare"}],
  "transferableSkills": [{"required": "GCP", "transferFrom": "Firebase + AWS-adjacent backend integration", "confidence": "medium"}],
  "missingSkills": ["GCP/AWS native cloud services"],
  "salaryEstimate": {"min": 4000, "max": 7000, "currency": "USD", "confidence": "medium"},
  "resumeEmphasis": ["Flutter Bond framework", "Cross-platform shipping", "Firebase + analytics integration", "Testing rigor"],
  "applicationTips": "Lead with Flutter Bond as proof of architectural depth..."
}

Rules:
- Be honest about misses. The role wants 2+ yrs Flutter, REST/gRPC, Firebase, App Store ship, CS fundamentals, and SOME cloud (GCP/AWS).
- The candidate has 7+ yrs mobile, deep Flutter (Bond framework, Trivia), but his cloud experience is mostly Firebase, not GCP/AWS native services. Capture that nuance.
...
// --- Step 2: Kareem (verbatim system + per-job prompt) ---------------------

async function generateResume(
  analysis: AIJobAnalysis
): Promise<ResumeGeneration> {
  // System prompt — mirrors resume-ai-agent.ts post-edit verbatim.
...
- Do NOT fabricate experience — only reframe existing experience`;
  ...
  // Featured-project + publication blocks — same construction Kareem uses.
...
// --- Step 3: render PDF via existing scripts/generate-pdf.js ---------------
...
async function main() {
  ...
  console.log(
    "  data/resumes/resume-Flutter-Application-Developer-1777396896450.pdf"
  );
}
