#!/usr/bin/env tsx
/**
 * Re-renders the JPMorgan iOS resume with hand-applied ATS-keyword edits
 * (no Claude calls, no drift from the spec).
 *
 * Usage:
 *   npx tsx scripts/render-jpmorgan-v2.ts
 */
  ...
  experienceEntries: [
    {
      ...
      bullets: [
        ...
        "Worked in Agile squads with bi-weekly releases — trunk-based development, Code Magic CI/CD running automated tests on every commit",
      ],
  ...
  skillsGrouped: [
    {
      ...
      items: ["Swift (object-oriented programming)", "Objective-C", "UIKit", "SwiftUI", "Combine", "Foundation", "Security", "Xcode", "iOS App Store"],
    },
    {
      category: "Testing & Quality",
      items: ["XCTest", "Unit Testing", "Memory Leak Detection", "Golden Tests", "TDD"],
    },
    {
...
function renderPdf(outputPdfPath: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const tmpJson = path.join(os.tmpdir(), `kareem-v2-${Date.now()}.json`);
    fs.writeFileSync(
      tmpJson,
...
async function main() {
  ...
  const resumeDump = path.join(os.tmpdir(), `test-jpmorgan-v2-${ts}-resume.json`);
  fs.writeFileSync(resumeDump, JSON.stringify(validated, null, 2));

  const pdfPath = path.join(outDir, `test-jpmorgan-v2-${ts}.pdf`);
